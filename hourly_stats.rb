    ...
    if is_request_valid?(request, serialized_requests)    
        ...
        if Hourly_stat.exists?(customer_id: request["customerID"])
            ...
            if Customer.exists?(request["customerID"])
                ...
                Hourly_stat.create(customer_id: "nonexistent", time: (request["timestamp"]), date: DateTime.strptime("#{(request["timestamp"])}",'%s').strftime("%d/%m/%Y"), request_count: 0 , invalid_count: 1)
            end
        end
    end   
end 